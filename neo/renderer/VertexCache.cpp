...
void idVertexCache::ActuallyFree( vertCache_t *block ) {
	...
	if ( block->tag != TAG_TEMP ) {
		...
		if ( block->vbo ) {
			...
			glBufferDataARB(GL_ARRAY_BUFFER, 0, 0, GL_DYNAMIC_DRAW_ARB);
#endif
	...
	*buffer = block;
	...
	if ( block->vbo ) {
		if ( indexBuffer ) {
			...
			glBufferDataARB( GL_ELEMENT_ARRAY_BUFFER, (GLsizeiptr)size, data, GL_STATIC_DRAW_ARB );
		} else {
			glBindBuffer( GL_ARRAY_BUFFER, block->vbo );
			if ( allocatingTempBuffer ) {
				glBufferDataARB( GL_ARRAY_BUFFER, (GLsizeiptr)size, data, GL_STREAM_DRAW_ARB );
			} else {
				glBufferDataARB( GL_ARRAY_BUFFER, (GLsizeiptr)size, data, GL_STATIC_DRAW_ARB );
			}
		}
	} else {
