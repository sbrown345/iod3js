...
vec3 FresnelSchlick( vec3 specularColor, vec3 L, vec3 H ) {   
    return specularColor + ( 1.0f - specularColor ) * pow(1.0f - clamp( dot( L, H ), 0.0, 1.0 ), 5 );   
}
...
vec4 lightingModel( vec4 diffuseTerm, vec3 specularTerm, vec3 L, vec3 N, vec3 H ) {   
	...
	vec3 specularLighting = max( FresnelSchlick(specularTerm, L, H), 0.0);  
...
void main( void ) {
	...
	//_specular = dot(_specular, N);
	vec3 falloff = pow(max((_specular - 0.75) * 4, 0.0), 2.0) * u_specularColor.rgb;
