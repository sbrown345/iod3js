...
class idLexer {
    ...
    // from Lexer.h:
    //private:
    	/*int			*/loaded:number;					// set when a script file is loaded from file or memory
    	/*idStr			*/filename:idStr;				// file name of the script
        /*int			*/  allocated: number;				// true if buffer memory was allocated
    	/*const char *	*/buffer:string;					// buffer containing the script
    	/*const char *	*/script_p:string;				// current pointer in the script
    	/*const char *	*/end_p:string;					// pointer to the end of the script
    	/*const char *	*/lastScript_p:string;			// script pointer before reading token
    	/*const char *	*/whiteSpaceStart_p:string;		// start of last white space
    	/*const char *	*/whiteSpaceEnd_p:string;		// end of last white space
    	/*ID_TIME_T		*/fileTime:number;				        // file time
    	/*int			*/length:number;					// length of the script in bytes
    	/*int			*/line:number;					// current line in script
    	/*int			*/lastline:number;				// line before reading token
    	/*int	    	*/tokenavailable:number;			// set by unreadToken
    	/*int			*/flags:number;					// several script flags
    	/*const punctuation_t *punctuations;		// the punctuations used in the script
    	/*int *			*/  punctuationtable: number;		// ASCII table with punctuations
        /*int *			*/nextpunctuation: number;		// next punctuation in chain
        /*idToken		*/token: idToken;					// available token
        /*idLexer *		*/next: idLexer;					// next script in a chain
    	/*bool			*/hadError:boolean;				// set by idLexer::Error, even if the error is supressed
                        
    	/*static char		*/baseFolder:string/*[ 256 ]*/;		// base folder to load files from
    ...
    ================
    */
    /*int */LoadMemory( /*const char **/ptr, /*int */length: number, /*const char **/name: string, /*int */startLine: number): number {
        if (this.loaded) {
