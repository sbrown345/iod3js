...
*/
	VolumeFromPolygon ( trm: R<idTraceModel>, /*float */thickness: number ): void {
		...
		for ( i = 0; i < this.numEdges; i++ ) {
			trm.$.verts[this.numVerts + i].opEquals( this.verts[i].opSubtraction( idVec3.opMultiplication_float( thickness, this.polys[0].normal ) ) );
			...
			for ( j = 0; j < poly.numEdges; j++ ) {
				...
				if ( edge.normal[0] == 0.0 && edge.normal[1] == 0.0 && edge.normal[2] == 0.0 ) {
					...
					if ( dot < idTraceModel.SHARP_EDGE_DOT ) {
						...
						edge.normal.opEquals( idVec3.opMultiplication_float( ( 0.5 / ( 0.5 + 0.5 * dot ) ), ( edge.normal.opAddition( poly.normal ) ) ) );
					}
				}
			}
		}
