...
class UseVertsInsteadOfThisIndexer {
	
}

class idSurface {
	[dontUseThis: string]: UseVertsInsteadOfThisIndexer;
...
////	int						GetNumVertices(void) const { return this.verts.Num(); }
////	const idDrawVert *		GetVertices(void) const { return this.verts.Ptr(); }
...
////
////protected:
	verts = new idList<idDrawVert>(idDrawVert);			// vertices
...
for ( var i = 0; i < 100; i++ ) {
	Object.defineProperty( idSurface.prototype, i.toString ( ), {
		get: function ( ): number {
			throw 'Do not use this indexer, use .verts[] instead';
		},
