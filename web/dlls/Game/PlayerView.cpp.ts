...
////	//
////	if ( lastDamageTime > 0.0 && SEC2MS( lastDamageTime ) + IMPULSE_DELAY > gameLocal.time ) {
...
////		float	armorPulse = ( gameLocal.time - player.lastArmorPulse ) / 250.0;
////
...
////
////		if ( alpha < 1.0  ) {
////			renderSystem.SetColor4( ( player.health <= 0.0 ) ? MS2SEC( gameLocal.time ) : lastDamageTime, 1.0, 1.0, ( player.health <= 0.0 ) ? 0.0 : alpha );
...
////		if ( player.GetInfluenceRadius() != 0.0 && distance < player.GetInfluenceRadius() ) {
...
////		renderSystem.DrawStretchPic( 0.0, 0.0, 640.0, 480.0, 0.0, 0.0, 1.0, 1.0, player.GetInfluenceMaterial() );
