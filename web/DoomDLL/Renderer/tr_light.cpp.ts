...
function R_SkyboxTexGen(surf: drawSurf_t, viewOrg: idVec3):void {
	...
	var /*int */numVerts = surf.geo.numVerts;
	var /*int */size = numVerts * sizeof( idVec3 );
	var /*idVec3 **/texCoords = newStructArray<idVec3>( idVec3, size );// (idVec3 *) _alloca16( size );

	var verts = surf.geo.verts;
...
=================
*/
function R_AddDrawSurf ( tri: srfTriangles_t, space: viewEntity_t, renderEntity: renderEntity_t,
	shader: idMaterial, scissor: idScreenRect ): void {
	var drawSurf: drawSurf_t;
	var shaderParms: Float32Array;
	var refRegs = new Float32Array( MAX_EXPRESSION_REGISTERS ); // don't put on stack, or VC++ will do a page touch
	var generatedShaderParms = new Float32Array( MAX_ENTITY_SHADER_PARMS );

	drawSurf = R_FrameAlloc<drawSurf_t>( drawSurf_t /*sizeof( *drawSurf )*/ );
	drawSurf.geo = tri;
	drawSurf.space = space;
	drawSurf.material = shader;
	drawSurf.scissorRect.opEquals(scissor);
	drawSurf.sort = shader.GetSort ( ) + tr.sortOffset;
	drawSurf.dsFlags = 0;

	// bumping this offset each time causes surfaces with equal sort orders to still
	// deterministically draw in the order they are added
	tr.sortOffset += 0.000001;
	...
	if ( tr.viewDef.numDrawSurfs == tr.viewDef.maxDrawSurfs ) {
		...
		tr.viewDef.drawSurfs = R_FrameAllocStructArray<drawSurf_t>( drawSurf_t, tr.viewDef.maxDrawSurfs );
		todoThrow( "memcpy( tr.viewDef.drawSurfs, old, count );" );
	}
	...
	if ( constRegs ) {
		...
		var regs = /*(float *)*/R_FrameAllocTypedArray<Float32Array>( Float32Array, shader.GetNumRegisters ( ) );
		drawSurf.shaderRegisters = regs;
		...
		var /*float */oldFloatTime: number;
		var /*int */oldTime: number;
	...
	case texgen_t.TG_SKYBOX_CUBE:
		R_SkyboxTexGen( drawSurf, tr.viewDef.renderView.vieworg );
		break;
	case texgen_t.TG_WOBBLESKY_CUBE:
		todoThrow ( );
	...
	if ( !space.entityDef ) {
		...
		var guiNum = shader.GetEntityGui ( ) - 1;
