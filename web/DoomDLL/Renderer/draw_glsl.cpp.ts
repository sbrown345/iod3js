...
var interactionShader = new shaderProgram_t;
var shadowShader = new shaderProgram_t;
var defaultShader = new shaderProgram_t;
var depthFillShader = new shaderProgram_t;
...
=================
*/
function R_LoadGLSLShader ( name: string, shaderProgram: shaderProgram_t, /*GLenum */type: number ): void {
	var fullPath = new idStr( "gl2progs/" + name );
	var /*char	**/fileBuffer = new R<Uint8Array> ( );
	var buffer: string;
...
=================
*/
function R_LinkGLSLShader ( shaderProgram: shaderProgram_t, needsAttributes: boolean ): boolean {
	//char buf[BUFSIZ];
	//int len;
	//GLint status;
	var linked: boolean;
	...
	linked = glGetProgramiv( shaderProgram.program, GL_LINK_STATUS );
...
=================
*/
function R_ValidateGLSLProgram(shaderProgram:shaderProgram_t ):boolean
{
	var validProgram:boolean;

	glValidateProgram(shaderProgram.program);

	validProgram = glGetProgramiv( shaderProgram.program, GL_VALIDATE_STATUS );
...
==================
*/
function R_ReloadGLSLPrograms_f ( args: idCmdArgs ): void {
	var /*int		*/i: number;
